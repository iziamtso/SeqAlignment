...
int main(int argc, char* argv[]) {
	...
	for (int i = 1; i < argc; ++i) {
		...
		if (std::string(argv[i]) == "--type" || std::string(argv[i]) == "-t") {
			...
			if (type != "single" && type != "many") {
				std::cerr << "The parameter passed to the 'type' is incorrect! It should be 'single' or 'many'." << std::endl;
				return 0;
			}
			continue;
		}
		...
		if (std::string(argv[i]) == "--outputFormat" || std::string(argv[i]) == "-of") {
			...
			if (format != "short" && format != "verbose") {
				std::cerr << "The parameter passed to the 'outputFormat' is incorrect! It should be 'short' or 'verbose'." << std::endl;
				return 0;
			}
	...
	std::cout << "Run time: " << (std::clock() - startTimer) / (double)(CLOCKS_PER_SEC / 1000) << " ms" << std::endl;
	cin.get();
	return 0;
}
